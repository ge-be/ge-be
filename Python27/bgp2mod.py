...
def dic_db(d): 
    ...
    for k in d:
        ins1 = "'" + k  + "', " + str(d[k])
        ins = 'insert into ' + tn + ' values (' + ins1 + ');'
#        print 'ins = ', ins
        cur.execute(ins)
...
class gls: 
    def __init__(self):
        self.pv = 2              # 2 - версия Python2  или любой
...
def prot_start(m, c = ''): 
    ...
    s = g.f()
    op = 'Start '
    print "prot_start : m = ", m, " op = ", op, " c = ", c
    if s[3] == 2:
        import bgp2mod
...
def prot_finish(m, c = ''): 
    ...
    s = g.f()
    op = 'Finish'
    print "prot_finish : m = ", m, " op = ", op, " c = ", c    
    if s[3] == 2:
        import bgp2mod
        f    = bgp2mod.prot_wr(m, op, c)
    else:
...
#    print "prot_wr : m = ", m, " op = ", op, " d_t = ", d_t, " c = ", c, " R = ", s[8]
    if s[8] == 1:
        import os
        if s[3] == 2:
            import bgp2mod
...
#        con.row_factory = sqlite3.Row
        cur = con.cursor() #создаём курсор - основной инструме
        ins = 'insert into ' + s[1] + '  values (' + "'" + m + "', '" + op 
        ins = ins +  "', '" + d_t + "', '"  +  c + "');"
#        print 'ins = ', ins
        cur.execute(ins)
        con.commit() #сохраняем изменения
...
#    print ('q = ', q, 'sm =', sm)    
    ...
    con.close()
    f2.close()
    if s[6] == 1: r = round( float(sm)/ float(q), 3)
    else:         r = 0.3
    mx =  0.15
    mn = -0.025
    if r > mx:           re = 'pos'
    ...
    if s[3] == 2:
        ...
        con = sqlite3.connect(s[0])#создаём соединение с базой данных
    cur = con.cursor()#создаём курсор - основной инструмент работы с БД
    ins1 = "'" + fn[1]  + "', " + str(q) + ", " + str(sm)
    ins1 = ins1 + ", " + str(r) + ", '" + re + "'"
    ins = 'insert into ' + tr + ' values (' + ins1 + ');'
#    print 'ins = ', ins
    cur.execute(ins)
...
# прогулка по каталогу для оценки тональности файлов 1 !!!
def walkt(p, r, d):
    import bgp2mod
    import os
#    import pdb
    ...
    m  = 'walk_t'
    f  = bgp2mod.table_ocv(m)# создание таблицы оценок вектора
    f  = bgp2mod.table_oc(m)# создание таблицы оценок 
    n  = 0
